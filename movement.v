...
fn (mut app App) move(x f32, y f32) {
	app.mvt_towards(app.move_dir(x, y))
}
...
fn (mut app App) mvts(x1 int, y1 int, x2 int, y2 int) { // xs and ys in block coords
	start_atype, start_x, start_y := block_to_array_coords(app.block_click_x, app.block_click_y) or {
		return
	}
	if app.check_array_occuped_in(start_atype, start_x, start_y) {
		stack_atype, stack_x, stack_y := block_to_array_coords(app.block_click_x + x1,
			app.block_click_y + y1) or { return }
		if app.check_array_occuped_in(stack_atype, stack_x, stack_y) {
			end_atype, end_x, end_y := block_to_array_coords(app.block_click_x + x2,
				app.block_click_y + y2) or { return }
